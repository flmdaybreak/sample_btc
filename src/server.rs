...
pub struct Server {
    node_address: String, // 节点的地址
    mining_address: String, // 矿工节点的地址
    inner: Arc<Mutex<ServerInner>>, //由于多线程相应请求所以使用Arc加Mutex保证线程安全
}
...
struct ServerInner {
    ...
    mempool: HashMap<String, Transaction>,  //挖矿节点的交易池，达到上限进行挖矿
}

const KNOWN_NODE1: &str = "localhost:3000"; //可以理解为全节点
...
impl Server {
    ...
    pub fn start_server(&self) -> Result<()> {
        ...
        thread::spawn(move || { //move闭包
            thread::sleep(Duration::from_millis(1000));
            if server1.get_best_height()? == -1 {  //本地没有区块数据，需要想网络中同步下载数据
                server1.request_blocks()
            } else {
                server1.send_version(KNOWN_NODE1)//向全节点发送版本号
            }
        });
    ...
    fn send_get_blocks(&self, addr: &str) -> Result<()> {
        ...
        let data = GetBlocksmsg {
            addr_from: self.node_address.clone(), //将自身节点地址放入消息中，用于表示自身，方便其他节点向自己发数据
        };
    ...
    ///  处理version请求，根据版本号和区块高度确定挥发消息类型。
    fn handle_version(&self, msg: Versionmsg) -> Result<()> {
        ...
        self.send_addr(&msg.addr_from)?;  //  广播节点
    ...
    ///  其它节点想要获取数据，判断节点需要是交易还是区块，并发送数据
    ...
    fn handle_tx(&self, msg: Txmsg) -> Result<()> {
        ...
        if self.node_address == KNOWN_NODE1 {  // 如果自身是全节点则向其其余节点广播接收到的交易hash
            ...
            if mempool.len() >= 2 && !self.mining_address.is_empty() { // 当内存池中的数据个数决定是否开始挖矿
                loop {
                    let mut txs = Vec::new();
...
///  根据不同的消息数据处理相应的连接请求
    fn handle_connection(&self, mut stream: TcpStream) -> Result<()> {
        ...
        println!("handle_connection and handling");
...
///   返回消息头（命令）  前CMD_LEN表示命令，后面表示数据，可以修改为 命令+数据大小+数据
